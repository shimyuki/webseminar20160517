package window.whiteboard {
    ...
    public class WbScrollBarH extends Sprite {
		private const TARGET_PADDING_RIGHT = 0;
		static public var SIZE = WbScrollBar.SIZE;
...
*/
			...
			m_bar.init( m_base.width - m_up.width - m_down.width - 2, scrollMask.width / scrollTarget.width);
		...
		// 同期モード用
		public function setScrollTargetPosi( _x:Number) {
			scrollTarget.x = _x;
			...
			var barW = m_bar.width;
			var rate = ( scrollMask.x - scrollTarget.x) / ( scrollTarget.width - scrollMask.width);
			m_bar.x = m_bar.getMinX() + ( baseW - barW) * rate;
		}
		
		// 同期モード用
		public function getScrollTargetPosi() : Number {
			return scrollTarget.x;
		}
		
		function onClickUp( e:MouseEvent) { if( contains( m_bar)) m_bar.setX( m_bar.x - 5);}
		function onClickDown( e:MouseEvent) { if( contains( m_bar)) m_bar.setX( m_bar.x + 5);}
		//function onClickBase( e:MouseEvent) { if( contains( m_bar)) m_bar.setX( m_bar.x + ( mouseX - m_bar.x)/2);}
...
//if( ExternalInterface.available) ExternalInterface.call( "flashFunc_alert", "SimpleScrollBar:update():" + String(scrollTarget.height) +","+ String(scrollMask.height) +","+ String(e.ratio));
			scrollTarget.x = m_scrollTarget_initX - ( scrollTarget.width - scrollMask.width) * e.ratio;
			dispatchEvent( new Event( "updated"));
		}
...
import flash.events.*;
import flash.geom.*;
import window.whiteboard.WbScrollBarH;
...
class ScrollBtnBase extends Sprite {
	...
	public function ScrollBtnBase() {
		...
		var ratios:Array = [0x00, 0xFF];
		var matr:Matrix = new Matrix();
		matr.createGradientBox( WbScrollBarH.SIZE-1, WbScrollBarH.SIZE , Math.PI/2, 0, 0);
		
		graphics.lineStyle( 1, 0xcccccc, 1, false, LineScaleMode.NONE);
		graphics.beginGradientFill( fillType, colors, alphas, ratios, matr, SpreadMethod.PAD);
		graphics.drawRect( 0, 0, WbScrollBarH.SIZE-1, WbScrollBarH.SIZE);
		graphics.endFill();
